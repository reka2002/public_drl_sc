...
def buildDeterministicMIP2(env, schedule=None, *args, **kwargs):
    # Build model
    m = ConcreteModel()
    ...
    def production_qty_constraint(m, j, t):
        if j != 0:
            return m.p[j, t] - m.run_rates[j] * m.y[j, t] == 0
        else:
    ...
    if reward == 'FULL':
        m.reward = Objective(expr=
            ...
            m.working_capital_per * sum(
                ...
                for i in m.i
                for t in m.t if t >= env.sim_time) - \
            sum(    # Add transition costs
                m.transition_costs[k, j] * m.z[j, k, t]
                for k in m.k
                for j in m.j
                for t in m.t if t >= env.sim_time),
            sense=maximize)
